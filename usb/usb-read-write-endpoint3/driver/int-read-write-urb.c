...
struct my_usb {
     ...
     //ENDPOINT IN - 3 - interrupt
     struct usb_endpoint_descriptor *int_in3_endpoint;
     uint8_t *int_in3_buf;
     struct urb *int_in3_urb;
};
...
int_cb(struct urb *urb)
{
   ...
   //This will show "A" sent from device on interrupt end point 1
   printk(KERN_ALERT "urb interrupt 1 - IN is finished: %s",
          sd->int_in_buf);
}

static void
int3_cb(struct urb *urb)
{
   struct my_usb *sd = urb->context;

   //This will show "B" sent from device on interrupt end point 3.
   printk(KERN_ALERT "interrupt endpoint 3 - IN is finished:%s",
          sd->int_in3_buf);
}
...
out_cb(struct urb *urb)
{
   ...
   // Sending data to device via INT - 2 is completed.
   printk(KERN_ALERT "urb interrupt 2 - OUT is finished");
}
          ...
          const char *buf, size_t count)
{
   ...
   //catch interrupt3 data from device
   ret = usb_submit_urb(sd->int_in3_urb, GFP_KERNEL);
   if (ret)
     {
        printk(KERN_ALERT "Failed to submit out urb");
     }
             ...
             const struct usb_device_id *id)
{
        ...
        if (endpoint->bmAttributes == 0x03)
          {
             ...
             else if (endpoint->bEndpointAddress == 0x83)
               {
                  data->int_in3_endpoint = endpoint;
               }
          }
     }
   ...
   data->int_in3_urb = usb_alloc_urb(0, GFP_KERNEL);
   ...
   data->int_in3_buf = kmalloc(le16_to_cpu(data->int_in3_endpoint->wMaxPacketSize), GFP_KERNEL);
   ...
   usb_fill_int_urb(data->int_in3_urb, udev,
                    usb_rcvintpipe(udev, data->int_in3_endpoint->bEndpointAddress),
                    data->int_in3_buf,
                    le16_to_cpu(data->int_in3_endpoint->wMaxPacketSize),
                    int3_cb,
                    data,
                    data->int_in3_endpoint->bInterval);
   ...
   ret = device_create_file(&interface->dev, &dev_attr_value);
   if (ret < 0)
     {
...
my_usb_disconnect(struct usb_interface *interface)
{
   ...
   // we are sending syncronous data via default control point
