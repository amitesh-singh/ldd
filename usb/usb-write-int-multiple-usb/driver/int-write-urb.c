...
struct my_usb {
     ...
     struct usb_endpoint_descriptor *int_out_endpoint2;
     uint8_t *int_out_buf2;
     struct urb *int_out_urb2;
};
...
static void
int2_cb(struct urb *urb)
{
   printk(KERN_ALERT "received data");
}
             ...
             const struct usb_device_id *id)
{
   ...
   struct usb_endpoint_descriptor *endpoint_val[2];
   struct my_usb *data;
   int i;
   unsigned x = 0;
   ...
   for (i = 0; i < iface_desc->desc.bNumEndpoints; i++)
     {
        ...
        if (endpoint->bEndpointAddress == 0x01)
          endpoint_val[x++] = endpoint;
        else if (endpoint->bEndpointAddress == 0x02)
          endpoint_val[x++] = endpoint;
   ...
   data->int_out_endpoint = endpoint_val[0];
   data->int_out_endpoint2 = endpoint_val[1];

   // allocate our urb for interrupt in 
   data->int_out_urb = usb_alloc_urb(0, GFP_KERNEL);
   data->int_out_urb2 = usb_alloc_urb(0, GFP_KERNEL);
   ...
   data->int_out_buf2 = kmalloc(le16_to_cpu(data->int_out_endpoint2->wMaxPacketSize), GFP_KERNEL);
   ...
   data->int_out_buf2[0] = 1;
   usb_fill_int_urb(data->int_out_urb2, udev,
                    usb_sndintpipe(udev, data->int_out_endpoint2->bEndpointAddress),
                    data->int_out_buf2,
                    le16_to_cpu(data->int_out_endpoint2->wMaxPacketSize),
                    int2_cb, // this callback is called when we are done sending/recieving urb
                    data,
                    (data->int_out_endpoint2->bInterval));
   ...
   i = usb_submit_urb(data->int_out_urb2, GFP_KERNEL);
   if (i)
     {
        printk(KERN_ALERT "Failed to submit urb");
     }
...
my_usb_disconnect(struct usb_interface *interface)
{
   ...
   usb_kill_urb(data->int_out_urb2);
   usb_free_urb(data->int_out_urb);
   usb_free_urb(data->int_out_urb2);
   kfree(data->int_out_buf);
   kfree(data->int_out_buf2);
