...
// In this example, we are having multiple interrupt OUT endpoints.
// check lsusb -s dbusno when you plug this device.
...
PROGMEM const char configDescr[] = {    /* USB configuration descriptor */
     ...
     USB_CFG_HAVE_INTRIN_ENDPOINT + USB_CFG_HAVE_INTRIN_ENDPOINT3 + 1 + 1, /* endpoints excl 0: number of endpoint descriptors to follow, 1 - is our OUT int , another 1 is our 2nd OUT int*/
     ...
     // out end point2 - interrupt out end point - ami
     7,          /* sizeof(usbDescrEndpoint) */
     USBDESCR_ENDPOINT,  /* descriptor type = endpoint */
...
/*
   USB_CFG_IMPLEMENT_FN_WRITEOUT must be defined
   to 1 to activate this feature. The endpoint number can be found in the
   global variable 'usbRxToken'.
 */
//usb_interrupt_msg or urb 
void usbFunctionWriteOut(uchar *data, uchar len)
{
   if (usbRxToken == 0x01)  // endpoint out 1
     {
   ...
   else if (usbRxToken == 0x02) // endpoint out 2
     {
        //do nothig?
     }
...
//usb_control_msg at default control point 0 comes here
