...
static void  _init_display(struct tft_device_data *tdd)
{
   ...
   tdd->send_data(tdd, 0x06); //color666 mode
...
static void write_3bytes(struct tft_device_data *tdd, uint32_t word)
{
   tdd->send_data(tdd, (word >> 16));
   tdd->send_data(tdd, (word >> 8) & 0xFF);
